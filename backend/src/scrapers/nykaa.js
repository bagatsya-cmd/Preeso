  ...
  async search(query, jobId = null) {
    ...
    return this.scrapeWithRetry(url, async (page) => {
      ...
      if (rawProducts.length === 0) {
        ...
        rawProducts = await page.evaluate((finalUrl, extractImgFn, disableValidation) => {
          const items = [];
          ...
          for (const linkEl of uniqueLinks.slice(0, 12)) {
            try {
              ...
              const imgResult = extractImg(card, 'Nykaa', window.location.href, disableValidation);
          ...
          return items;
        }, finalUrl, extractImageInBrowser.toString(), disableImgValidation);
      }
