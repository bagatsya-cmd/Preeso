...
async function runAggregation(job) {
  ...
  const formattedRaw = rawScraped.map((doc) => {
    if (process.env.DISABLE_IMAGE_VALIDATION === 'true' && doc.image) {
      console.log(`[IMAGE_SAVED]\nstage=scraped_product_db_read\nretailer=${doc.source}\nproduct=${doc.title}\nimage=${doc.image}`);
    }
    return {
      title: doc.title,
      price: doc.price,
  ...
  if (process.env.DISABLE_IMAGE_VALIDATION === 'true') {
    groupedCards.forEach(card => {
      if (card.imageUrl) {
        console.log(`[IMAGE_SAVED]\nstage=aggregation\nretailer=${card.stores?.[0]?.storeName || 'aggregated'}\nproduct=${card.baseName}\nimage=${card.imageUrl}`);
      }
      (card.stores || []).forEach(st => {
        if (st.image) {
          console.log(`[IMAGE_SAVED]\nstage=aggregation\nretailer=${st.storeName}\nproduct=${st.originalName || card.baseName}\nimage=${st.image}`);
        }
      });
    });
  }
  ...
  if (process.env.DISABLE_IMAGE_VALIDATION === 'true' && savedResult && savedResult.products) {
    savedResult.products.forEach(card => {
      if (card.imageUrl) {
        console.log(`[IMAGE_SAVED]\nstage=product_result_db_write\nretailer=${card.stores?.[0]?.storeName || 'aggregated'}\nproduct=${card.baseName}\nimage=${card.imageUrl}`);
      }
      (card.stores || []).forEach(st => {
        if (st.image) {
          console.log(`[IMAGE_SAVED]\nstage=product_result_db_write\nretailer=${st.storeName}\nproduct=${st.originalName || card.baseName}\nimage=${st.image}`);
...
async function startWorker() {
  if (!SCRAPING_ENABLED) {
    console.log('[AggregatorWorker] Scraping disabled. Worker will not start.');
    return;
  }
