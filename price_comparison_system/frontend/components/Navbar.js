...
/* ── Pricio inline SVG logo — no file dependency ─────────────────────────── */
function PricioFullLogo({ height = 36 }) {
  return (
    <svg viewBox="0 0 180 44" height={height} fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Pricio">
      {/* Shopping bag */}
      ...
      {/* Wordmark */}
      <text x="36" y="32" fontSize="20" fontWeight="800" fill="white" fontFamily="Poppins,Inter,sans-serif" letterSpacing="-0.5">Pricio</text>
      <defs>
...
function PricioIconLogo({ size = 32 }) {
  return (
    <svg viewBox="0 0 44 44" width={size} height={size} fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Pricio icon">
...
export { PricioFullLogo, PricioIconLogo };
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const userData = localStorage.getItem('pricio_user')  || localStorage.getItem('comparex_user');
  ...
  const handleLogout = () => {
    ['pricio_token','pricio_user','comparex_token','comparex_user'].forEach(k => localStorage.removeItem(k));
    setUser(null);
    router.push('/');
  };
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1440, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 66 }}>
        ...
        {/* ── Logo ── */}
        <Link href="/" className="logo-wrapper" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
          <span style={{
